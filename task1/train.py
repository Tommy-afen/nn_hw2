...
def train(args):
    ...
    for epo in range(args.epochs):
        ...
        writer.add_scalars(f"{args.aug if args.aug else 'non-aug'}/loss", {'train_loss': train_loss, 'val_loss': val_loss},
                          epo + 1)
        writer.add_scalars(f"{args.aug if args.aug else 'non-aug'}/acc", {'train_acc': train_acc, 'val_acc': val_acc},
                          epo + 1)
    ...
    if not args.aug:
        torch.save(model, 'models_save/model.pt')
    else:
        torch.save(model, f'models_save/model_{args.aug}.pt')
